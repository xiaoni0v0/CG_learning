...
def visualize_bezier_circle(num_segments=4):
    ...
    # 绘制曲线
    plt.plot(curve_points[:, 0], curve_points[:, 1], 'b-', linewidth=2, label='Bezier Circle')
    
    # 绘制控制点
    plt.plot(control_points[:, 0], control_points[:, 1], 'ro', markersize=6, label='Control Points')
    ...
    plt.plot(true_circle_x, true_circle_y, 'g--', alpha=0.5, linewidth=1, label='True Circle')
    ...
    plt.xlabel('X')
    plt.ylabel('Y')
    plt.savefig(f'bezier_circle_{num_segments}_segments.png', dpi=150, bbox_inches='tight')
    plt.show()
